...
if not os.path.exists("scores_benchmarking.npz"):
    ...
    score_cp = []
    score_orgseg = []
    for f_gt, f_morgana, f_cp, f_orgseg in tqdm.tqdm(
        zip(
            list_gt[:n_max],
            list_morgana[:n_max],
            list_cp[:n_max],
    ...
    np.savez(
        "scores_benchmarking.npz",
        morgana=score_morgana,
        cellprofiler=score_cp,
        organoseg=score_orgseg,
    )
