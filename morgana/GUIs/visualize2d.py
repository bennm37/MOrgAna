...
class visualization_2d(QWidget):
    ...
    def remakePlot(self):
        ...
        n_rows = [
            [len(self.data[group_idx][gastr_idx]) for gastr_idx in range(n_gastr[group_idx])]
        ...
        if self.YnormBtn.currentText() == "Global":
            ...
            for i in range(n_groups):
                for j in range(n_gastr[i]):
                    for k in range(n_rows[i][j]):
                        data[i][j][k] = np.clip(
                            (data[i][j][k] - percs[0]) / (percs[1] - percs[0]),
                            0.0,
                            1.0,
                        )
            ...
            for i in range(n_groups):
                for j in range(n_gastr[i]):
                    for k in range(n_rows[i][j]):
                        percs = np.percentile(np.array(data[i][j][k]), (0.3, 99.7))
                        data[i][j][k] = np.clip(
                            (data[i][j][k] - percs[0]) / (percs[1] - percs[0]),
                            0.0,
                            1.0,
    ...
    def save_tifs(self):
        titles = ["Save mean data of group1", "Save std data of group1"]
            ...
            if name != "":
                ...
                if os.path.splitext(name)[-1] != ".tif":
                    buttonReply = QMessageBox.question(
                        self,
                        "File format warning!",
                        "File format not recommended. Do you want to save the image as tif?",
                    )
