...
class visualization_1d(QWidget):
    ...
    def remakePlot(self):
        ...
        if self.YnormBtn.currentText() == "Global":
            ...
            for i in range(n_groups):
                for j in range(n_gastr[i]):
                    data[i][j] = np.clip((data[i][j] - percs[0]) / (percs[1] - percs[0]), 0.0, 1.0)
            ...
            for i in range(n_groups):
                for j in range(n_gastr[i]):
                    percs = np.percentile(np.array(data[i][j]), (0.3, 99.7))
                    data[i][j] = np.clip((data[i][j] - percs[0]) / (percs[1] - percs[0]), 0.0, 1.0)
            ...
            if self.groupPlotBtn[i].isChecked():
                ...
                for k in range(n_gastr[i]):
                    ...
                    if self.rawBtn.isChecked():
                        ax.plot(ydata_group[-1], "-", lw=0.5, c=colors[i], alpha=0.2)
