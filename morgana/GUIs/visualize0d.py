...
class visualization_0d(QWidget):
    ...
    def remakePlot(self):
        ...
        if self.YnormBtn.currentText() == "Global percentile":
            ...
            for i in range(n_groups):
                data[i] = np.clip((data[i] - percs[0]) / (percs[1] - percs[0]), 0.0, 1.0)
        elif self.YnormBtn.currentText() == "Group percentile":
            for i in range(n_groups):
                percs = np.percentile(np.array(data[i]), (0.3, 99.7))
                data[i] = np.clip((data[i] - percs[0]) / (percs[1] - percs[0]), 0.0, 1.0)
        ...
        for i in range(n_groups):
            if self.groupPlotBtn[i].isChecked():
                ...
                if self.plotType.currentText() == "bar":
                    ...
                    if data[i] != []:
                        parts = ax.boxplot(data[i], positions=[i], notch=True, patch_artist=True)
                        for item in [
                            "boxes",
                            "whiskers",
                            "fliers",
                        ...
                        plt.setp(parts["boxes"], alpha=0.5)
                ...
                if self.rawBtn.isChecked():
                    ...
                    ax.plot(x, data[i], "ok", alpha=0.7, ms=2)
