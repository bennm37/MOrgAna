...
def compute_profiles_fluo(fl, ma, m, visualize=False):
    ...
    fl_straight = np.reshape(
        map_coordinates(
            fl,
            np.reshape(m, (m.shape[0] * m.shape[1], 2)).T,
            order=0,
            mode="constant",
            cval=0,
        ).T,
    ...
    radProf = np.array([0.0 for i in range(int(np.sqrt(fl.shape[0] ** 2 + fl.shape[1] ** 2)))])
    radCount = np.array([0.0 for i in range(int(np.sqrt(fl.shape[0] ** 2 + fl.shape[1] ** 2)))])
    angProf = np.array([0.0 for i in range(360)])
    angCount = np.array([0.0 for i in range(360)])
    idxs = np.where(ma > 0)
