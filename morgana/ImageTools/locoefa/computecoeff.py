...
def compute_LOCOEFA(mode, DEBUG=False):
    ...
    # Equation 14: tau1
    mode.tau[1] = 0.5 * np.arctan2(
        2.0 * (mode.alpha[1] * mode.beta[1] + mode.gamma[1] * mode.delta[1]),
    ...
    ) + mode.beta[1 : N_modes + 1] * np.sin(np.arange(1, N_modes + 1) * mode.tau[1])
    mode.betastar[1 : N_modes + 1] = -mode.alpha[1 : N_modes + 1] * np.sin(
        np.arange(1, N_modes + 1) * mode.tau[1]
    ) + mode.beta[1 : N_modes + 1] * np.cos(np.arange(1, N_modes + 1) * mode.tau[1])
    mode.gammastar[1 : N_modes + 1] = mode.gamma[1 : N_modes + 1] * np.cos(
        np.arange(1, N_modes + 1) * mode.tau[1]
    ) + mode.delta[1 : N_modes + 1] * np.sin(np.arange(1, N_modes + 1) * mode.tau[1])
    mode.deltastar[1 : N_modes + 1] = -mode.gamma[1 : N_modes + 1] * np.sin(
        np.arange(1, N_modes + 1) * mode.tau[1]
    ) + mode.delta[1 : N_modes + 1] * np.cos(np.arange(1, N_modes + 1) * mode.tau[1])
    ...
    for i in range(1, N_modes + 1):
        # Equation 26: phi
        mode.phi[i] = 0.5 * np.arctan2(
            2.0 * (mode.a[i] * mode.b[i] + mode.c[i] * mode.d[i]),
        ...
        mode.lambdaminus[i] = (mode.lambda1[i] - mode.lambda2[i]) / 2.0
    ...
    if DEBUG:
        ...
        for i in range(N_modes + 2):
            print(
                "LOCO-EFA mode %d:\tlambdaplus=%f\tlambdaminus=%f\tzetaplus=%ftzetaminus=%f\n"
                % (
                    i,
                    mode.locolambdaplus[i],
                    mode.locolambdaminus[i],
                    mode.locozetaplus[i],
                    mode.locozetaminus[i],
                )
            )
    ...
    mode.locoL[1:] = np.sqrt(
        mode.locolambdaplus[1:] * mode.locolambdaplus[1:]
        + mode.locolambdaminus[1:] * mode.locolambdaminus[1:]
        + 2.0
        * mode.locolambdaplus[1:]
        * mode.locolambdaminus[1:]
        * np.cos(mode.locozetaplus[1:] - mode.locozetaminus[1:] - 2.0 * mode.locozetaplus[1])
    )
...
def compute_LOCOEFA_Lcoeff(mask, down_shape=1.0, N_modes=50, DEBUG=False):

    ma_down = transform.resize(
        mask.astype(float),
        (int(mask.shape[0] * down_shape), int(mask.shape[1] * down_shape)),
        order=0,
        preserve_range=True,
    )
...
if __name__ == "__main__":
    ...
    for mm in tqdm.trange(max_mode, desc="Max_modes"):
        ...
        x_loco, y_loco = ImageTools.locoefa.reconstruct.reconstruct_contour(
            mode, tp=tp, rec_type="LOCOEFA", first_mode=0, last_mode=mm
        )
        x_efa, y_efa = ImageTools.locoefa.reconstruct.reconstruct_contour(
            mode, tp=tp, rec_type="EFA", first_mode=0, last_mode=mm
        )
