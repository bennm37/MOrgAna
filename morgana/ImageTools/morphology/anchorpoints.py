...
def compute_anchor_points(ma, _slice, down_shape, smoothing=1000):
    ...
    ### find contour
    points = find_contours(ma_down, 0.0)[0]
    ...
    if midlinePoints.shape[0] == 1:
        # midline = np.where(a)
        midlinePoints = np.array(
            [
                [midlinePoints[0][0] - 1, midlinePoints[0][1] + 1],
                [midlinePoints[1][0] - 1, midlinePoints[1][1] + 1],
            ]
        )
        midlinePoints = np.transpose(midlinePoints)
    ...
    anch = np.concatenate(
        (np.array([edge_point_L]), anchors[::s], np.array([edge_point_R])),
        axis=0,
    ).astype(float)
