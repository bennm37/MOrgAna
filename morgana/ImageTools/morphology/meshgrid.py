...
def visualize_meshgrid(
    ...
    if ma:
        ax.imshow(ma, "red", interpolation="none", alpha=0.3)
    ax.plot(midline[:, 1], midline[:, 0], "w", lw=1.0)
    if show_tangent:
        ax.quiver(
            midline[::N_step, 1],
            midline[::N_step, 0],
    ...
    for mp in meshgrid[::20]:
        ax.plot(mp[::20, 1].flatten(), mp[::20, 0].flatten(), "-ow", lw=0.1, ms=0.2)
    ax.axis("off")
    # ax.set_xlim(0,550)
    # ax.set_ylim(0,550)
