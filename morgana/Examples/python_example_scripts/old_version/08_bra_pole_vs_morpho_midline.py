...
if __name__ == "__main__":
    ...
    for gastr in gastr_names:
        ...
        if not os.path.exists(bra_fname):
            ...
            df_straight = ioStr.load_straight_morpho_params(save_folder, gastr)
            df_fluo = ioFluo.load_fluo_info(save_folder, gastr)

            N_img = len(df_morpho.input_file)

            x_morphopole = np.array([0.0 for i in range(N_img)])
            y_morphopole = np.array([0.0 for i in range(N_img)])
            axratios = np.array([0.0 for i in range(N_img)])

            x_fluomax = np.array([0.0 for i in range(N_img)])
            y_fluomax = np.array([0.0 for i in range(N_img)])
            alphas_fluomax = np.array([0.0 for i in range(N_img)])

            x_fluocm = np.array([0.0 for i in range(N_img)])
            y_fluocm = np.array([0.0 for i in range(N_img)])
            alphas_fluocm = np.array([0.0 for i in range(N_img)])

            times = np.array([0.0 for i in range(N_img)])
            ...
            for i in tqdm.trange(N_img):
                ...
                ax1[0].plot(
                    [centroid[1], centroid_fluo[1]],
                    [centroid[0], centroid_fluo[0]],
                    "--g",
                )
