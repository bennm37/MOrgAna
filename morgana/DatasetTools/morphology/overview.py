...
def generate_overview_finalMask(input_folder, chosen, saveFig=True, downshape=1, autoclose=False):
    ...
    for i in tqdm.tqdm(range(n_img)):
        ...
        ax[i].imshow(masks[i], cmap=cmap, interpolation="none", alpha=0.3)
    ...
    if saveFig:
        ...
        fig.savefig(
            os.path.join(input_folder, "result_segmentation", cond + "_finalMasks.png"),
            dpi=75,
        )
        print("### Done saving!")
...
def generate_composite_movie_cropped(input_folder):
    ...
    for i in tqdm.tqdm(range(n_imgs)):
        ...
        center = [
            int((_slice[0].stop + _slice[0].start) / 2),
        ...
        _slice_large = [
            slice(
                int((center[0] - max_dim[0] / 2)),
                int((center[0] + max_dim[0] / 2)),
                None,
            ),
            slice(
                int((center[1] - max_dim[1] / 2)),
                int((center[1] + max_dim[1] / 2)),
                None,
            ),
        ]
    ...
    imsave(
        os.path.join(segment_folder, cond + "_composite_recap.tif"),
...
def generate_composite_img_cropped(input_folder, downshape=1, keep_open=True):
    ...
    for i in tqdm.tqdm(range(n_img)):
        ...
        cmaps = [
            mpl.colors.LinearSegmentedColormap.from_list("my_cmap", ["black", i], 256)
            for i in [
                "white",
                "lime",
                "red",
        ...
        for j in range(len(imgs)):
            ax[i, -1].imshow(
                ...
                cmap=cmaps[j],
                interpolation="none",
                vmin=np.percentile(imgs[j], 1.0),
                vmax=np.percentile(imgs[j], 99.0),
                alpha=alphas[j],
            )
            ax[i, j].imshow(
                imgs[j],
...
def generate_meshgrid_img_cropped(input_folder, keep_open=True):
    ...
    for i in tqdm.tqdm(range(n_img)):
        ...
        ax[i].contour(ma, [0.5], colors="r", alpha=0.5)
        ax[i].plot(anch[:, 1], anch[:, 0], "-or", lw=0.5, ms=0.5, alpha=0.5)
