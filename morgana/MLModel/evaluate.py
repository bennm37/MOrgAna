...
def evaluate(model_folder):
    ...
    flist_in = ioDT.get_image_list(
        os.path.join(model_folder, "testset"), string_filter="_GT", mode_filter="exclude"
    )
    flist_GT = ioDT.get_image_list(
        os.path.join(model_folder, "testset"), string_filter="_GT", mode_filter="include"
    )
    ...
    if params["model"] == "unet":
        ...
        img_watershed = [predict.make_watershed(predict.make_mask(c[0]), c[1][:, :, 2]) for c in img_classifier]
